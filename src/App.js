...
import GameApp from './bughunt-game'
import LeaderboardDisplay from './bughunt-leaderboard'

export default function App() {
  const [screen, setScreen] = useState('home')

  if (screen === 'game') return <GameApp />
  if (screen === 'leaderboard') return <LeaderboardDisplay />
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 36, letterSpacing: 8, fontWeight: 900 }}>BUG HUNT</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: 280 }}>
        <button onClick={() => setScreen('game')} style={{
          background: 'transparent', border: '1px solid #00e5ff', color: '#00e5ff',
          fontFamily: 'monospace', fontSize: 13, letterSpacing: 3, padding: '14px 24px',
